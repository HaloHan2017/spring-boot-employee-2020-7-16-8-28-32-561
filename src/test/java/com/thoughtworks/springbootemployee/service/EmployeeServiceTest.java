...
class EmployeeServiceTest {
    ...
    @Mock
    private EmployeeResponseMapper mockedEmployeeResponseMapper;

    private List<Employee> getEmployees() {
        return Arrays.asList(new Employee(2, "bruno", 20, "male", 2000),
    ...
    void should_return_updated_employee_when_update_given_employee_id_and_employee_info() throws NoSuchDataException {
        ...
        given(mockedEmployeeResponseMapper.toEmployeeResponse(any())).willReturn(new EmployeeResponse(1, "lisi", 15, "female", 12200));
    ...
    void should_return_1_when_add_employee_given_employee() {
        ...
        given(mockedEmployeeResponseMapper.toEmployeeResponse(any())).willReturn(new EmployeeResponse(11, "tom", 49, "male", 1000));
        // when
    ...
    void should_return_employees_when_get_employees_by_gender_given_gender() {
        // given
        given(mockedEmployeeRepository.findByGender("male")).willReturn(getEmployees());
        // when
    ...
    void should_return_employees_when_get_all_employee_given_no_parameter() {
        // given
        given(mockedEmployeeRepository.findAll()).willReturn(getEmployees());
        // when
        List<EmployeeResponse> employees = mockedEmployeeService.getAllEmployees();
        // then
        assertEquals(2, employees.size());
    }
    ...
    void should_return_employees_when_get_employees_by_range_given_page_and_pageSize() {
        ...
        given(mockedEmployeeRepository.findAll(PageRequest.of(1, 5))).
                willReturn(new PageImpl<>(getEmployees()));
        // when
